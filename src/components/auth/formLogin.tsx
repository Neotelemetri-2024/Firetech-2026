...
export default function FormLogin() {
    ...
    const panelStyles = darkMode
        ? "border-slate-800 bg-red-900/80"
        : "border-slate-200 bg-white/85";

    const inputStyles = darkMode
        ? "border-slate-700 bg-slate-950/70 text-white placeholder:text-slate-500 focus:border-red-700 focus:ring-red-400/20"
        : "border-slate-300 bg-white text-slate-900 placeholder:text-slate-400 focus:border-red-500 focus:ring-red-500/20";
      ...
      >
        ...
        <div className="p-6 sm:p-8">
          <div className="mb-6 flex items-center gap-3">
            <div className="flex h-12 w-12 items-center justify-center rounded-2xl bg-red-700 text-white shadow-[0_18px_40px_rgba(139,92,246,0.34)]">
              <ShieldCheck className="h-6 w-6" />
            </div>
            <div>
              <p className="text-xs font-semibold uppercase tracking-[0.2em] text-red-700 dark:text-red-700">
                Login
              </p>
              <h2 className="text-2xl font-black">Masuk ke akun</h2>
            </div>
          </div>
          ...
          {success && (
            <div className="mb-4 flex items-center gap-3 rounded-2xl bg-green-500/20 p-4 border border-green-500/50">
              ...
              <p className="text-sm font-semibold text-green-700 dark:text-green-300">
                Login berhasil! Silakan lanjut ke dashboard.
              </p>
            </div>
          )}
          ...
          <form onSubmit={handleLoginSubmit} className="space-y-5">
            ...
            <div className="flex flex-col gap-3 text-sm sm:flex-row sm:items-center sm:justify-between">
              <label className="flex items-center gap-3 z-100 ">
                <input
                  type="checkbox"
                  className="h-4 w-4 rounded border-slate-300 accent-red-700 cursor-pointer"
                />
                <span
              ...
              <a
                href="#"
                className="font-semibold underline text-red-700 hover:text-red-800 dark:text-red-700 dark:hover:text-red-600"
              >
                Lupa password?
              </a>
            </div>
            ...
            <button
              ...
              className="inline-flex h-12 w-full  items-center justify-center gap-2 rounded-2xl bg-red-700 px-4 font-semibold text-white transition hover:-translate-y-0.5 hover:bg-red-600 disabled:cursor-not-allowed disabled:opacity-60 disabled:hover:translate-y-0 cursor-pointer"
            >
              {loginLoading ? (
                <>
